...
sub createPostUnitiggerConsensusJobs (@) {
    ...
    if ($consensusType eq "cns") {
        ...
        print F "\$bin/convertToPBCNS -path $blasr -consensus $consensusType -coverage 1 -threads 4 -prefix $wrk/5-consensus/$asm.\$jobid.tmp -length 500 -sequence $wrk/5-consensus/$asm.\$jobid.fasta -input $wrk/5-consensus/$asm.\$jobid.lay -output $wrk/5-consensus/$asm.\$jobid.fa\n";
